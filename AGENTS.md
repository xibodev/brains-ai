...
- Any test that could affect the installed Brains service, database, state directory, ports, or client configuration must run in Docker with isolated synthetic state and configuration. Never mount or mutate the operator's local Brains or harness state for testing.
