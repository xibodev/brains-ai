param([string]$Name = "brains-opencode-lifecycle-probe")
...
if ($status) { throw "OpenCode lifecycle probe requires an exact clean committed worktree." }
...
try {
  ...
  if ($LASTEXITCODE -ne 0) { throw "OpenCode lifecycle probe image build failed." }
  $createdImage = $true

  $createdContainer = $true
  & docker run --name $container --cap-drop ALL `
    --security-opt no-new-privileges:true `
    --tmpfs "/tmp:rw,exec,nosuid,nodev,mode=1777" `
    --tmpfs "/home/node:rw,exec,nosuid,nodev,uid=1000,gid=1000,mode=0700" `
    $image /opt/brains-venv/bin/python /opt/uat/opencode_lifecycle_probe.py
  if ($LASTEXITCODE -ne 0) { throw "OpenCode lifecycle probe failed." }
} finally {
  if ($createdContainer) {
    & docker rm -f $container 2>$null | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "OpenCode lifecycle probe container teardown failed." }
  }
  if ($createdImage) {
    & docker image rm $image 2>$null | Out-Null
    if ($LASTEXITCODE -ne 0) { throw "OpenCode lifecycle probe image teardown failed." }
  }
}
