...
def _make_private(path: Path) -> None:
    ...
    acl = subprocess.run(
        [
            ...
            "-NonInteractive",
            "-Command",
            # Read the ACL through .NET rather than Get-Acl: the cmdlet's module
            # costs an analysis pass that repeats in full whenever the caller's
            # per-user cache location is empty, which is the case in this
            # deliberately isolated home. These types need no module at all.
            "$ErrorActionPreference = 'Stop'; "
            "$target = $env:BRAINS_NATIVE_WAKEUP_ACL_PATH; "
...
def _bootstrap(arguments: argparse.Namespace) -> int:
    ...
    if os.name == "nt":
        # The worker redirects USERPROFILE into the private root so nothing
        # reaches the operator's profile. Windows then creates its per-user tree
        # there. That is an artifact of this probe's own isolation rather than
        # state it failed to clean up, so remove it before asserting what remains.
        for name in ("AppData", "Application Data", "Local Settings"):
            residue = invocation / name
            if residue.is_dir():
                shutil.rmtree(residue, ignore_errors=True)
    leftovers = sorted(path.name for path in invocation.iterdir())
    if environment.exists() or leftovers != [output.name]:
        # These names are generated by this probe, so listing them names the
        # residue without disclosing anything about the host.
        raise RuntimeError(f"native invocation cleanup is incomplete: {leftovers[:10]}")
...
def main() -> int:
    ...
    try:
        ...
        if arguments.child_action:
            ...
            # paths. Its curated RuntimeError text names a contract only. An
            # OSError contributes its numeric codes, which identify the refusal
            # without quoting the filename the message would carry.
        ...
        # other exception contributes only its class name, plus the basename of
        # the tool that stalled, which is a system executable rather than a host
        # value and is the only way to tell a hang apart from a rejection.
        if isinstance(exc, RuntimeError) and str(exc):
            detail = str(exc)[:200]
        elif isinstance(exc, subprocess.TimeoutExpired):
            raw = exc.cmd[0] if isinstance(exc.cmd, (list, tuple)) and exc.cmd else exc.cmd
            detail = f"TimeoutExpired:{Path(str(raw)).name}"
        else:
