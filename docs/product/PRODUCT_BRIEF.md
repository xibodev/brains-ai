...
tables added by migration 155. Requester acknowledgement is automatic; all other required
members must acknowledge the canonical version/hash before collection. Blinding applies
...
readable. Deadline expiry only flags incomplete state on reads, without automatic
execution or settlement. Cancellation affects protocol state, not assignments, processes
or mail. Context and links are inert, with no autogenerated assignment integration.
This availability makes no frontend or readiness promise; see
