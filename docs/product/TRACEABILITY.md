...
| F3 Sessions and HITL | P1, P4, P5, P7 | J7, J8, J11; AC-F3-01..07 | Inspect Sessions, route/resolve decisions, report friction, coordinate continuity, stop when supported | `/app/governance`, `/app/coordination`, `/app/workspaces/:slug`; Labs Session evidence at `/app/labs/sessions*` | operator feedback/governance/coordination; Session commands; approval route/escalate/resolve; typed event scope; `/v1/ws`; `/v1/events` | leased Session lifecycle, event taxonomy/scope inference, mailbox/successor continuity, feedback, decisions, realtime | `agent_sessions`, `events`, `event_contexts`, continuity/routing/feedback rows; 121/123/132/133/142-147 | Event-context, feedback, lease, successor and decision-routing tests exist. Browser E4 feedback promotion and shipped CLI interactive input remain gaps. |
...
| B2 Coordination/MCP | P1, P3, P7 | J5-J8, J10; AC-B2-01..04 | Start/end, discover, task/claim/handoff/message, feedback report/enrich, topic, checkpoint/resume | MCP SSE/stdio, CLI, `/app/coordination`, `/app/workspaces*`, `/app/act` | feedback inbox, Session leases/successors, interest-scoped topics, fenced ephemeral review, typed coordination adapters | coordination, feedback, knowledge, tasks/claims/mailbox/topics/successors/leases/review executions; 139/140/142-149 | Feedback, async/ephemeral help, topic-delivery and Session-continuity suites cover scope, source isolation and amplification; real provider/remote-Runtime review and stdio trust remain incomplete. |
...
| B4 Governance/audit | P5, P6, P7 | J8, J10, J11; AC-B4-01..04 | Request, assign/escalate, resolve approval, execute, verify audit | `/app/governance`, typed HTTP, MCP/CLI governance families, relay | human-bound approval routing plus canonical governed-action contract, decisions, bridges, audit | approvals, `approval_routing`, `governed_actions`, `audit_log`, `audit_chain_head`; 070, 126-128, 145 | Decision-routing and governance tests cover assignment, overdue/escalation, resolution separation and the chain; the boundary remains in-process, so out-of-band third-party effects stay outside it. |
...
| B6 CLI/wiring/service | P1, P4, P6 | J1, J2, J9, J11; AC-B6-01..04 | Install, setup, serve, wire, manage service | `brains-ai`; PyPI; Copilot/Claude/Codex/OpenCode wire adapters; `/app/operations` posture | exact-interpreter verifier, persisted endpoint selection/probes, native MCP renderers, supervisor bind preflight | package metadata, agent config, service endpoint config, and PID state | `test_wire.py` covers native schema, preservation, conflict, permissions, idempotency, status and unwire; `test_service.py` / `test_supervisor.py` cover explicit-port refusal, fallback persistence and fail-fast bind behavior; browser host mutations remain disabled until typed preview/confirmation contracts exist. |
...
| B8 Observability/readiness | P4, P6 | J7, J11; AC-B8-01..04 | Probe liveness/readiness, inspect bounded operational posture, diagnose queues | `/app/operations`, `/health`, protected admin/operator projections, logs, optional OTLP | health, readiness, queue diagnosis/repair, recovery policy, censored welcome follow-through, supervisor, runtime sweep | traces, typed events, usage, process log files | Adoption reports expose observation/action windows and eligible denominators and explicitly measure follow-up events rather than outcomes; `/health` remains liveness only, and service logs and deployment-specific recovery evidence are not browser contracts yet. |
...
| Runtimes | register, heartbeat, list/get/patch/offline, enroll, assignments, ephemeral help-review list/claim/complete, Session/event ingest, Session-command poll/claim/ack/release, Session reconcile | Runtime-narrow credential for its own machine, or operator `org.read`/`org.write`/`org.admin`; help reviews require the Runtime's exact tool, Org, healthy online state, and a registered working-root alias before a deadline-fenced claim; a Session command may only be listed, claimed and settled by the Runtime its Session is bound to - sharing a machine is not ownership, the machine recorded on the row is a diagnostic rather than the ownership test, and a command whose Session is unbound belongs to the local process; release accepts only the current holder; token redeem unauthenticated | F1, F3, B2 |
