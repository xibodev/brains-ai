...
| B2 Coordination/MCP | P1, P3, P7 | J5-J8, J10; AC-B2-01..04 | Start/end, task/claim/handoff/message, feedback, topic, help, checkpoint/resume | MCP SSE/stdio, CLI, `/app/coordination`, `/app/workspaces*`, `/app/act` | feedback inbox, Session leases/successors, durable address delivery/read/thread controls, browser mailbox desk, interest-scoped topics, fenced ephemeral review, typed coordination adapters | coordination, feedback, knowledge, tasks/claims/mailbox/thread/message/delivery/topics/successors/leases/review executions; 139/140/142-150 | Advertised plus BL-P1-15/20 experiments. Local offline mailbox E3 and container-only browser selector/Inbox/Sent/thread/compose/reply/forward/deep-link E4 exist; live notification/SMTP, real-provider review, and stdio trust remain incomplete. |
...
| Session/state | advertised start/heartbeat/end, state, event, snapshot, checkpoint/resume, mailbox register/phonebook/lookup/send/broadcast/reply/forward/inbox/sent/thread; execution message/stop commands are withdrawn | advertised coordination Session/state/event/snapshot and matching mailbox families; execution message/stop commands are withdrawn | B2, F3 |
...
| Coordination, durable mail, and withdrawn execution compatibility | `registered_tools`, `agent_sessions`, `events`, durable mailbox/thread/message/delivery/notification/SMTP/legacy-inventory rows, plus withdrawn `runtimes`, `enrolment_tokens`, `personas`, `projects`, `issues`, `issue_comments`, `skills`, attachment, and `session_commands` rows | 100, 120-125, 133, 138, 150 | Coordination Session/tool/event rows remain advertised. Mailbox identity, operator inbox provisioning, one-current attachment/cursor, phonebook/lookup authorization, lifecycle detach, direct/offline acceptance, explicit broadcast, Inbox/Sent, thread/reply/forward, per-recipient read state, and Coordination browser UI are implemented. Notification/SMTP behavior remains open. Migration 150 inventories legacy mail/tool links as unverified rather than fabricating addresses. Runtime/Persona/Project/Issue/Skill/execution-command rows are compatibility inventory; source behavior and historical attribution do not authorize activation. BL-P0-09 must preserve only the data needed to open existing stores. |
