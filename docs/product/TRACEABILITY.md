...
| CLI/wiring/service | `cli`, `wire`, `service`, supervisor | Install, wire Copilot/Claude/Codex/OpenCode, and operate Brains | B6 | Resolved command/group/callback/parameter contracts and every actual per-harness generated file/transport field are reviewed in both startup modes; the shared public context builder preserves Streamable HTTP `/mcp`, legacy SSE `/sse`, and stdio distinctions. Browser host mutation contracts remain absent. |
...
| Legacy web | no implementation remains | Deleted browser surface | B9 | Source, route, command, package-data, wheel, and sdist checks prevent reintroduction; an entry-rooted SPA import graph rejects reachable unknown/frozen modules and targets while separately inventorying dormant retained source. Only modern SPA authentication endpoints remain. |
