...
| CLI/wiring/service | `cli`, `wire`, `service`, supervisor | Install, wire Copilot/Claude/Codex/OpenCode, and operate Brains | B6 | Resolved command/group/callback/parameter contracts and actual per-harness generated files are reviewed in both startup modes; the shared public context builder preserves Streamable HTTP `/mcp`, legacy SSE `/sse`, and stdio distinctions. Browser host mutation contracts remain absent. |
