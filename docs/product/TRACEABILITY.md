...
| Operator console | `/v1/operator/*` overview, Workspace control rooms, coordination, governance, operations, capabilities, scoped mutations, mailbox registration/phonebook/lookup, audit verification | resolved operator principal plus per-Workspace `org.read`/`org.write`; mailbox registration additionally requires a caller-owned live Session and binding header; install operations/global approvals require bootstrap admin | F0, F3, F7, F9, B2, B4-B6, B8 |
...
| Session/state | advertised start/heartbeat/end, state, event, snapshot, checkpoint/resume, mailbox register/phonebook/lookup; execution message/stop commands are withdrawn | advertised coordination Session/state/event/snapshot and mailbox register/phonebook/lookup; execution message/stop commands are withdrawn | B2, F3 |
...
| Coordination, durable mail, and withdrawn execution compatibility | `registered_tools`, `agent_sessions`, `events`, durable mailbox/thread/message/delivery/notification/SMTP/legacy-inventory rows, plus withdrawn `runtimes`, `enrolment_tokens`, `personas`, `projects`, `issues`, `issue_comments`, `skills`, attachment, and `session_commands` rows | 100, 120-125, 133, 138, 150 | Coordination Session/tool/event rows remain advertised. Mailbox identity, operator inbox provisioning, one-current attachment/cursor, phonebook/lookup authorization, and lifecycle detach are implemented; delivery/read/thread/notification/SMTP behavior remains open. Migration 150 inventories legacy mail/tool links as unverified rather than fabricating addresses. Runtime/Persona/Project/Issue/Skill/execution-command rows are compatibility inventory; source behavior and historical attribution do not authorize activation. BL-P0-09 must preserve only the data needed to open existing stores. |
