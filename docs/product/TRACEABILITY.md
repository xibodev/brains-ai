...
| Legacy web | no implementation remains | Deleted browser surface | B9 | Source, route, command, package-data, wheel, and sdist checks prevent reintroduction; a pinned TypeScript-compiler AST graph follows imports, re-exports, and dynamic imports from the real entry and rejects unknown/frozen modules, expression or concatenation bypasses, and unresolved navigation sinks while separately inventorying dormant retained source. Runtime-provided destinations pass through a pinned core-route guard. Only modern SPA authentication endpoints remain. |
| Containers/ingress/CI | Dockerfiles, compose, deploy, workflows | Reproducible candidate and operations | B5, B6, B8 | Every CI job is blocking; normalized wheel/sdist members, package metadata, configuration keys, SPA navigation, public IDs, CLI/MCP/HTTP surfaces, and supported harness output must exactly match the reviewed manifest. Deploy scaffold consistency and candidate-specific hosted evidence remain separate concerns. |
