...
| F7 Config | P1, P2, P6 | J9; AC-F7-01..04 | Inspect supported local config; edit only approved non-secret settings | `/app/operations/config/local`; `Config` | `GET/PUT /v1/operator/configuration` | `brains.control.configuration`; config loader and local service controls | `tests/test_core_configuration.py`; `j09-config-settings.spec.ts` | Advertised. The positive manifest is redacted, bootstrap-admin-only, attributable, atomic, CAS-protected, restart-explicit, and the sole browser/API configuration writer. |
...
| UM-07 | Modern Config edits supported state. | Advertised for approved rate-limit and SQLite writes with explicit restart outcomes and recovery. Frozen fields are absent and the deleted legacy browser provides no alternate writer. | F7, J9 |
