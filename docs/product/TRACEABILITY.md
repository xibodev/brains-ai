...
| J11 | `j11-console-clean.spec.ts`, `j11-operator-web-hardening.spec.ts` | auth, error, WS, privacy tests | Route/state, non-disclosing not-found, connection-degradation, two-width overflow, and modal keyboard/focus contracts are present; multi-process realtime evidence remains open. |
