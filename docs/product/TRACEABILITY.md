...
| B2 Coordination/MCP | P1, P3, P7 | J5-J8, J10; AC-B2-01..04 | Start/end, task/claim/handoff/message, fixed notification claim/settle, feedback, topic, help, checkpoint/resume | MCP SSE/stdio, CLI, `/app/coordination`, `/app/workspaces*`, `/app/act` | feedback inbox, Session leases/successors, durable address delivery/read/thread controls, proof-bound body-free notification protocol, browser mailbox desk, interest-scoped topics, fenced ephemeral review, typed coordination adapters | coordination, feedback, knowledge, tasks/claims/mailbox/thread/message/delivery/notification/topics/successors/leases/review executions; 139/140/142-151 | Advertised plus BL-P1-15/20 experiments. Local offline mailbox E3, fixed-nudge take/settle with pull fallback, and container-only browser selector/Inbox/Sent/thread/compose/reply/forward/deep-link E4 exist; concrete harness hook/plugin installation, SMTP, real-provider review, and stdio trust remain incomplete. |
...
| Coordination, durable mail, and withdrawn execution compatibility | `registered_tools`, `agent_sessions`, `events`, durable mailbox/thread/message/delivery/notification/SMTP/legacy-inventory rows, plus withdrawn `runtimes`, `enrolment_tokens`, `personas`, `projects`, `issues`, `issue_comments`, `skills`, attachment, and `session_commands` rows | 100, 120-125, 133, 138, 150-151 | Coordination Session/tool/event rows remain advertised. Mailbox identity, operator inbox provisioning, one-current attachment/cursor, phonebook/lookup authorization, lifecycle detach, direct/offline acceptance, explicit broadcast, Inbox/Sent, thread/reply/forward, per-recipient read state, Coordination browser UI, and the proof-bound fixed-nudge notification state machine are implemented. `wire` remains pull-only because no hook/plugin is installed; external harness wakeup and SMTP remain open. Migration 150 inventories legacy mail/tool links as unverified rather than fabricating addresses; migration 151 constrains notification modes and attempt transitions. Runtime/Persona/Project/Issue/Skill/execution-command rows are compatibility inventory; source behavior and historical attribution do not authorize activation. BL-P0-09 must preserve only the data needed to open existing stores. |
...
| J8 | `j08-governance-session-control.spec.ts` | approval/ask/gate plus durable mailbox authorization/delivery tests | Browser coverage asserts one-time governance resolution, fail-closed legacy session-control navigation, and container-only mailbox selector/read/compose/reply/forward/reload/responsive journeys. |
