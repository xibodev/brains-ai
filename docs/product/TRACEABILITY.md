...
| B2 Coordination/MCP | P1, P3, P7 | J5-J8, J10; AC-B2-01..04 | Start/end, task/claim/handoff/message, local notification claim/settle, help, checkpoint/resume | MCP Streamable HTTP `/mcp` by default, stdio, explicit legacy SSE compatibility, CLI, consented Claude Code stop hook, `/app/coordination`, `/app/workspaces*`, `/app/act` | Session leases/successors, explicit native-ID validation/extraction controls, pinned OpenCode global lifecycle plugin, CLI/MCP managed versioned mailbox bindings with hash-only recovery intents, durable address delivery/read/thread controls, proof-bound body-free local notification protocol, recoverable managed Claude settings mutation, browser mailbox desk, typed coordination adapters | coordination, knowledge, tasks/claims/mailbox/thread/message/delivery/notification/successors/leases; topic/feedback/SMTP/review rows are frozen inventory | Advertised for durable local coordination. Identity storage and lifecycle controls persist raw adapter provenance and fail closed across invalid input, conflicts, restart/resume, Workspace aliases, binding rotation/recovery/revocation, and abrupt exit. Every OpenCode plugin installation path verifies version 1.18.25 before mutation; the owned plugin uses an absolute Brains interpreter/module command and a minimal state environment. Its real-binary probe covers first turn, forced exit/expiry/recovery, explicit nonzero movement conflict, retryable deletion finalization, revocation, old-proof rejection, `PATH` shadowing, and environment minimization. The consented Claude hook emits only the constant nudge, bounds reclaim/uncertainty, preserves exact prior settings through recoverable exchange, and resolves only a current proof-bound attachment; Copilot CLI, Codex, and OpenCode remain pull-only notification adapters. Owner-only binding files use POSIX modes or Windows DPAPI plus an exact current-user DACL. BL-P1-12 remains open for native-platform exchange and recovery-state evidence. Topic boards, feedback intelligence, pattern routing, SMTP, and peer-review admission are frozen. |
...
| B6 CLI/wiring/service | P1, P4, P6 | J1, J2, J9, J11; AC-B6-01..04 | Install, setup, serve, wire, manage service | `brains-ai`; PyPI; Copilot/Claude/Codex/OpenCode wire adapters; consented Claude mailbox stop hook; `/app/operations` posture | verified windowless Windows launcher, exact-interpreter verifier, persisted endpoints/probes and Brains-confined service identity, native MCP/hook renderers, supervisor bind preflight and per-child listener watchdog | package metadata, agent config, service endpoint config, PID state | Advertised/partial. E3 includes a hermetic Windows/macOS/Linux service-manager lifecycle crossed with every wire adapter, readiness-gated install compensation, retained-definition failure paths, KeepAlive-safe macOS stop, Windows owned-tree uninstall, unchanged Linux linger policy, and reversible consented Claude hook wiring with truthful pull fallback. CI installs the built wheel on each native OS/Python combination and checks clean-home setup, native definition rendering, persistence, and reversible wiring. Native hook recovery-state evidence remains BL-P1-12 work. A guarded manual manager-cycle scaffold exists, but actual native scheduler login/reboot execution is still E4 and BL-P0-06 remains open until disposable-host runs pass. |
...
The checker also requires an explicitly supplied fresh build directory and compares the
current normalized product source plus exactly one wheel and one sdist with the reviewed
manifest; validation and manifest generation fail closed when those inputs are absent or
...
| CLI/wiring/service | `cli`, `wire`, `service`, supervisor | Install, wire Copilot/Claude/Codex/OpenCode, and operate Brains | B6 | Resolved Click command/group and normalized public command/option/argument activation fields plus every actual per-harness generated file/transport field are reviewed in both startup modes. Independent semantic validation fixes each managed path, creation action, sentinel/guidance body, authentication/process content, and Streamable HTTP `/mcp`, legacy SSE `/sse`, or stdio distinction. Browser host mutation contracts remain absent. |
...
| Legacy web | no implementation remains | Deleted browser surface | B9 | Source, route, command, package-data, wheel, and sdist checks prevent reintroduction. A pinned TypeScript-compiler graph starts at the real entry and enforces a finite navigation boundary: `App.tsx` alone declares literal routes and `coreRoutes.tsx` alone owns router and DOM navigation capability. Reachable source rejects JSX attribute spreads; React/DOM element factories and JSX-runtime reexports; dynamic imports, `require`, and `import.meta`; dynamic-code constructors, reflective metaprogramming, and string or unknown timer handlers; router aliases; ambient-root escape; DOM attribute mutation and programmatic activation; and DOM-capability escape. Ordinary data spreads, finite computed data indexing, statically callable callbacks, and reviewed typed constructors remain available. Typed builders guard runtime destinations and the reviewed per-file hash inventory makes every reachable source change explicit. Run `node scripts/inventory_spa_surface.mjs frontend/src`; a healthy probe exits zero and emits the reachable module graph plus the normalized navigation inventory. Dormant retained source remains separately inventoried. Only modern SPA authentication endpoints remain. |
| Containers/ingress/CI | Dockerfiles, compose, deploy, workflows | Reproducible candidate and operations | B5, B6, B8 | Every CI job is blocking; the package job builds and explicitly supplies fresh artifacts before normalized product-source and exact wheel/sdist members, package metadata, configuration keys, SPA navigation, public IDs, CLI/MCP/HTTP surfaces, and supported harness output are compared with the reviewed manifest. Deploy scaffold consistency and candidate-specific hosted evidence remain separate concerns. |
...
5. Frontend routes and route parameters, API client calls, mounted server routes and their families, SQLAlchemy entities, the migration corpus, journey specs, acceptance tests, and `AC-*` references are generated from source by `scripts/check_traceability.py` and fail the gate on drift. The separate reviewed positive core-surface manifest seals the current normalized product source, freshly built wheel and sdist, discoverable and activatable CLI/MCP/HTTP/SPA/configuration/wiring/package inventories, plus actionable command, path, link, fenced-command, and named-surface claims in canonical product documentation in normal and all-opt-in modes. Explicit frozen, withdrawn, deleted, historical, and compatibility boundary prose remains documentable without becoming an advertisement.
