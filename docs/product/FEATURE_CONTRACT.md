...
| Acceptance criterion | Target contract | Current status |
|---|---|---|
| AC-F3-01 | Session events are durably stored and backfilled before realtime continuation. | present (E1/E2/E3) for event storage/backfill. Migration 147 adds one `event_contexts` row per event with typed category, Workspace/global/unresolved scope, and inference provenance; new events infer Workspace from Session or known entity metadata, and legacy Session events are backfilled. Unscoped extension events remain accepted but explicitly unresolved and hidden from non-admin event listings. Session, Issue, approval and Runtime realtime events also commit before announcement and replay by cursor, while transcript chunks stay notification-only and are backfilled over REST. |
| AC-F3-02 | Session state follows explicit spawning, running, dormant, blocked, completed, and failed transitions. | present (E1/E2/E3); process-bound execution Sessions retain terminal reconciliation, while PID-less coordination Sessions carry renewable leases, become non-terminal `dormant` after silence, release claims/tasks, and can be resumed without falsifying an execution failure. |
...
| AC-F3-04 | Asks and approvals appear in Governance and can be resolved once with related Workspace/Session context. | present/partial (E1/E2/E3); the Workspace-scoped queue distinguishes human answers from approve/reject/defer decisions. Migration 145 adds human-owned assignment, priority, due time, and reasoned escalation metadata. Routing requires a browser/local human with Workspace write access and an assignee in the approval's Org; it never resolves or authorizes the action. Complete publish coverage remains incomplete. |
...
**Status:** present/partial (E1/E2/E3). The surface is broad. MCP SSE resolves the presented key to one principal and refuses Runtime-narrow credentials. PID-less coordination Sessions carry renewable leases; `session-heartbeat` / `brains_heartbeat_session` renew without journal noise, the scheduler makes expired handles `dormant`, and CLI/MCP start reuses the sole matching Workspace/tool/operator handle or refuses ambiguity. A distinct replacement automatically links the freshest predecessor and transfers its claims, in-progress tasks, and topic subscriptions; successor reads preserve unread predecessor mail without rewriting recipients. Topic posts create one durable announcement rather than one mailbox row per live Workspace: a Session explicitly subscribes, `inbox_wait` wakes when that topic advances, and a Session-scoped `topic_read` advances its durable per-topic cursor oldest-first. Direct mailbox reads and waits accept a client-held message-id high-water mark, and an empty mailbox poll no longer records a false `message_read` adoption event. An immediately repeated exact checkpoint payload reuses the latest cairn, and an exact `set_handoff` retry reuses the active handoff; both return `duplicate: true`, renew liveness, and suppress a second row/event, while an intervening payload or lifecycle transition remains durable history. Harness-qualified peer help has an asynchronous durable lifecycle: file returns a code immediately, get/wait observe it later, a bounded wait timeout leaves it open, the requester may cancel, and the current claimant may release it for another peer; the original `ask_peer` remains a blocking compatibility wrapper. `/app/coordination`, `/app/workspaces/:slug`, and `/app/act` expose scoped reads and named HTTP mutations over the same controls. Topic names remain install-wide and `required_tool` on a post remains advisory. Duplicate suppression is transactional for sequential retries but has no dedicated uniqueness key for concurrent identical writers. Approval resolution remains separated from the requester. Stdio MCP still inherits the launching process's trust boundary, and per-tool destructive-action governance remains incomplete.
...
Approval routing is a separate human-governed metadata path. `approval_routing` records an optional Org-member assignee, priority, due time, and monotonic reasoned escalation level; `decision-route` / `decision-escalate` and matching HTTP/MCP surfaces require a browser/local human plus Workspace write visibility and commit with `approval.routed` / `approval.escalated` audit entries. Overdue routing is advisory queue health only: it never approves, rejects, defers, expires, consumes, or otherwise makes an action executable.
...
**Status:** present/partial (E1/E2/E3). Wiring has conflict-safe, idempotent status/unwire adapters for Copilot CLI, Claude Code, Codex, and OpenCode; each renderer uses the tool's native MCP schema, preserves unrelated settings, backs up edits, and marks only its own entry removable. Service installation uses and verifies the exact `sys.executable` that can import Brains rather than guessing a sibling `pythonw.exe`; install fails loud when the interpreter probe fails. It also preflights the requested gateway port, refuses an unavailable explicit port, or persists a bindable fallback when the default is unavailable. `service.status()` reports the effective console/MCP endpoints, PID identity, and bounded listener probes, and is healthy only when the installed service both owns a process and serves. The supervisor preflights every enabled listener on its actual bind host, holds a bounded degraded state while a bind is blocked, and then exits with a configuration code the systemd unit excludes from restart; installation refuses a gateway/MCP port pair the supervisor would reject. PID verification additionally accepts an exact Brains `serve-all` command-line match when Windows start-time reporting is unavailable/inconsistent, while still refusing unrelated processes. Legacy installers remain outside the supported path.
