...
|---|---|---|---|---|---|---|
| F0 | A user can operate Brains from one coherent console and start attributable work. | Sign in -> select Org -> navigate -> select work or receive an explicit blocker -> Spawn -> inspect Session. | `/app`, Inbox, Personas, Sessions; Org/Persona/Session APIs and controls. | Org and Persona creation events; `spawn_session_opened`; `spawn_enqueued`; Session state, Org, Persona, Runtime and Issue links. | BL-P0-05, BL-P3-01 | AC-F0-01..05; J1, J3, J7, J11 |
...
| F3 | A user can supervise a durable Session, intervene, approve, steer, stop and resume. | Dispatch -> watch transcript -> answer/approve/chat -> stop or complete -> return later. | Sessions and Inbox; Session/event/decision APIs; WS/SSE; CLI/MCP Session, decision, checkpoint and resume families. | Explicit lifecycle transitions; persisted events; `decision_filed` and `decision_resolved`; delivery acknowledgement for message/stop; scoped reconnect backfill. | BL-P0-02, BL-P0-03, BL-P0-05, BL-P0-07, BL-P1-12 | AC-F3-01..07; J7, J8, J11 |
...
| F7 | A user can understand effective Brains configuration and change only settings exposed by an approved write contract. | Open Config -> inspect redacted state -> test provider -> remain read-only or use an explicitly supported write/reload/rollback path. | Config screen; config summary/provider-test APIs; config loader and provider registry; conditional approved admin writes. | Redacted effective-state result; provider readiness result; for supported writes, attributable audit plus reload and rollback outcome. | BL-P1-05, BL-P1-11, BL-P2-01 | AC-F7-01..04; J9 |
...
| F9 | An owner can manage an Org, member roles, scope and attributable usage. | Create/select Org -> add member -> assign role -> restrict access -> inspect scoped usage. | Settings and Org switcher; Org/member/usage APIs and identity controls. | Principal and role attribution; Org/Workspace authorization decision; membership events; product-entity usage dimensions. | BL-P0-01, BL-P0-02, BL-P0-07, BL-P1-07 | AC-F9-01..05; J10, J11 |
