...
The coordination plane carries four comms primitives alongside tasks/claims/handoffs. `inbox_wait` is the single polling surface for direct mail, subscribed topics, and peer help:
...
- **Direct + workspace mail** — existing `send_message`/`read_messages`; a message addressed via `workspace_path` reaches whatever Session is alive in that Workspace. Pass `after_id` (MCP) / `--after-id` (CLI) with the highest processed message id to exclude an old prefix. Empty reads return `[]` without appending a `message_read` event; that adoption event now means at least one message was actually returned.
- **Peer help** — `file_help_request` returns an open durable request and code immediately; `get_help_request` reads it, and `wait_help_request` waits a bounded interval without expiring it merely because that caller stopped waiting. `wait_for_request` claims matching work, `answer_request` settles it with required evidence, the requester can `cancel_help_request`, and a claimant can `release_help_request` back to open with a refreshed deadline. Matching `brains-ai help-file|get|wait|claim|answer|cancel|release|list` commands expose the lifecycle outside MCP. The original existing-peer lane remains blocking. `required_tool="claude"` or `"not:copilot"` remains case-insensitive; a non-matching harness cannot claim the request.
- **On-demand reviewers** — MCP `ask_peer`/`file_help_request` and CLI `help-file` accept `execution_mode=existing|auto|ephemeral`. `auto` is the MCP/CLI default only when an exact `required_tool` and Workspace target make a launch safe: a live peer gets a short claim window, then migration 149's fenced execution row can be claimed by one matching healthy Runtime or by the hub when that CLI is installed locally. `ephemeral` launches immediately; Session targets and `not:<tool>` remain existing-peer only. Auto/ephemeral calls return the durable request code immediately and are observed with `wait_help_request`.
- **Review source boundary** — an ephemeral reviewer never receives the registered source path. Brains inventories Git-tracked files, fingerprints tracked content plus Git status, copies the tracked snapshot and current diff into a temporary directory, makes that snapshot read-only where the host supports file modes, and launches the CLI there with provider-specific read-only/no-write tool policy and a temporary HOME. The parent re-fingerprints the registered source before accepting the answer; any change discards the output. Snapshot mutation is harmless and destroyed with the sandbox. Review output is redacted and bounded; execution is leased, attempt-fenced, limited to three retries, attributable to a terminal ephemeral Session, and recorded through the governed subprocess boundary. This is not a claim of universal child-process/network confinement: provider-native sandbox enforcement and credential isolation vary by tool/host, and controlled real-Runtime operation remains E4 evidence.
- **One poll primitive** — `inbox_wait` (MCP) / `brains-ai inbox-wait` blocks until the Session has mail newer than optional `after_message_id` / `--after-message-id`, a subscribed topic advanced, or a claimable peer request. A topic wake repeats until a Session-scoped topic read acknowledges it by advancing the stored cursor. Cadence remains operator/agent policy; this makes each tick latency-bounded instead of sleep-guessed.
...
- **Retry deduplication** — an immediately repeated exact `checkpoint` call reuses that Session's latest cairn, and an immediately repeated exact `set_handoff` call reuses the Workspace's active handoff. The retry returns the original id with `duplicate: true`, renews Session liveness, and emits no second row/event. Any intervening checkpoint payload or handoff lifecycle/payload change creates durable history normally. This is sequential retry idempotency; concurrent identical writers have no dedicated uniqueness key.
- **Topic boards** — `topic_post`/`topic_read`/`topic_list` plus `topic_subscribe`/`topic_unsubscribe`/`topic_subscriptions` (MCP), with matching `brains-ai topic-*` commands. Topics are install-wide named boards and posts may thread via `reply_to`. A notifying post creates one `topic_announcements` row, never a mailbox row per Workspace. Only live Sessions explicitly subscribed to that topic wake, and never for a post from their own Workspace. `topic_read(topic, session_id=...)` returns unseen posts oldest-first and advances the durable per-topic cursor; ordinary board reads remain newest-first and do not require a subscription. `--no-blast` stores only the board post. `required_tool` remains advisory; enforced claiming is exclusive to peer help.

Topic posts remain an append-only archive like knowledge entries. Their announcement/subscription cursors are separately inventoried by queue health as ephemeral delivery state; orphaned cursor rows are deterministic integrity-repair candidates, while the posts themselves are preserved.
...
|---|---|---|
| `approvals` | the assigned human resolver, or the visible human queue when unassigned | indefinite while open; optional `due_at` makes overdue work visible but never auto-resolves it |
...
| `event_contexts` | the event producer; bootstrap operator for unresolved scope | one context per event, following event retention; unresolved scope is operational debt, not silently global |
