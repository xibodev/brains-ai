...
- **Peer help** — `file_help_request` returns an open durable request and code immediately; `get_help_request` reads it, and `wait_help_request` waits a bounded interval without expiring it merely because that caller stopped waiting. `wait_for_request` claims matching work, `answer_request` settles it with required evidence, the requester can `cancel_help_request`, and a claimant can `release_help_request` back to open with a refreshed deadline. Matching `brains-ai help-file|get|wait|claim|answer|cancel|release|list` commands expose the lifecycle outside MCP. The original `ask_peer` remains a blocking compatibility wrapper. `required_tool="claude"` or `"not:copilot"` remains case-insensitive; a non-matching harness cannot claim the request.
