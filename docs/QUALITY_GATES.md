...
- The reviewed positive core-surface manifest is generated independently for normal and all-opt-in startup. Exact inventories cover resolved CLI commands, groups, callbacks, option spellings and parameter contracts; advertised MCP names and callable contracts; HTTP operations; navigation and redirects across the full SPA source; supported configuration keys; actual generated harness files; package entry points/dependencies/extras; canonical IDs; and normalized, regular-file-only wheel/sdist members. Missing, extra, malformed, or uninspectable inventory fails closed; mutation tests prove each family cannot silently expand.
