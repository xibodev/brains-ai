...
- The reviewed positive core-surface manifest is generated independently for normal and all-opt-in startup. Exact inventories cover resolved CLI commands, groups, callbacks, every option spelling and parameter field; advertised MCP names and callable contracts; HTTP operations; the import graph rooted at the real SPA entry, reachable navigation/redirects, and separately retained dormant frontend source; supported configuration keys; every generated harness file and transport contract; package entry points/dependencies/extras; canonical IDs; and normalized, regular-file-only wheel/sdist members. Semantic allowlists reject reachable unknown or frozen SPA modules and targets even if the manifest is regenerated. Missing, extra, malformed, or uninspectable inventory fails closed; field-level mutation tests prove each discoverability family cannot silently expand.
