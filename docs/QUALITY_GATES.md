...
- The reviewed positive core-surface manifest is generated independently for normal and all-opt-in startup. Exact inventories cover CLI commands and aliases, MCP tools and contracts, HTTP operations, SPA routes/navigation/palette, supported configuration keys, harness wiring, package entry points/dependencies/extras, canonical IDs, and normalized wheel/sdist members. Missing, extra, malformed, or uninspectable inventory fails closed; mutation tests prove each family cannot silently expand.
